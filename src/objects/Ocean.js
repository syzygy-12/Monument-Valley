export default class Ocean {
    constructor({ size, speed, signalIdList, levelManager }) {
      ...
      for (let i = 0; i < this.geometry.attributes.position.count; i++) {
        ...
        this.amplitudes[i] = Math.random() * 1 + 0.5; // 每个顶点有不同的波动幅度
      }
    }
    ...
    tick(delta) {
      ...
      for (let i = 0; i < this.geometry.attributes.position.count; i++) {
        ...
        positions[index + 2] = Math.sin((x + z) * frequency + time * this.speed + randomOffset) * amplitude; // Y 位置（海浪的高度）
      }
      ...
      this.geometry.attributes.position.needsUpdate = true;
        ...
        if (this.animationType === "translation") {
